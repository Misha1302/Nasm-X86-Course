...
- понимать `ZF/SF/CF/OF` из дня 11;
- читать порядок операндов `destination, source`.

Если один из пунктов не получается объяснить или показать на маленьком примере, вернись к [Дню 11](/day_11).

---

## За 30 секунд

- `cmp a,b` выставляет флаги как `a-b`, но не сохраняет результат.
- `test a,b` выставляет флаги как `a&b`, но не сохраняет результат.
- Signed-переходы `jl/jg` нельзя заменять unsigned-переходами `jb/ja`.
- Между `cmp/test` и `jcc` нельзя бездумно ставить команду, которая меняет флаги.

## Минимум после главы

Ты должен уметь:

- перевести простое условие C++ в `cmp/test + jcc`;
- выбрать signed или unsigned переход;
- проверить число на ноль и отдельный бит;
- прочитать порядок операндов `cmp` без инверсии.

Можно пока не заучивать:

- таблицу всех редких `jcc`;
- ручной вывод формул флагов для каждого сравнения.
...
## Сквозной пример: три проверки

Будем возвращаться к одному условию на протяжении главы:
...
```text
проверить x != 0
→ проверить младший бит x
→ signed-сравнить x и y
→ выполнить тело только если все проверки прошли
```
...
В следующих разделах отдельно разберём, какие флаги создаёт каждая проверка и почему здесь нужны именно `je` и signed `jge`.
...
1. Реши [TR-12 в рабочей тетради](/transfer_workbook#tr-12) без просмотра ответа.
2. После законченной попытки открой [диагностический ключ TR-12](/transfer_keys#key-tr-12).

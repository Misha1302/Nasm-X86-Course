...
::: warning
Это дополнительный материал. Не проходи его до CDECL, фреймов, массивов и выравнивание.
:::
...
Здесь `fprintf` — указатель на функцию, а `stdout, "%d\n"` — дополнительные аргументы, которые надо передать дальше.

## Что такое указатель на функцию на уровне NASM
...
## Расположение аргументов CDECL для `apply`
...
Если `n = 2`, то дополнительные аргументы таковы:
...
2. потом положить дополнительные аргументы в обратном порядке: `vararg[n-1] ... vararg[0]`;
3. вызвать `fn`;
4. убрать `(n + 1) * 4` байт аргументов.

## Почему дополнительные аргументы нельзя «просто передать»
...
Если `fn` — библиотечная функция, перед `call fn` тоже нужно 16-байтное выравнивание стека.
...
| положить дополнительные аргументы в прямом порядке | функция прочитает аргументы наоборот |
...
| забыть выравнивание | Spring-04 может падать |

## Мини-практика

Для `n = 2` и дополнительных аргументов:

...
- отличать signed-интерпретацию от unsigned;
...
- `movzx` нужен для unsigned, `movsx` — для signed.
...
- объяснить разницу zero extension и sign extension;
- подготовить signed dividend через `cdq`.
...
- [ ] объяснить, почему signed/unsigned — это интерпретация битов;

...
Оцени каждый пункт только после завершения экзаменационного режима:

| Блок | Баллы | Время | Что проверяет |
|---|---:|---:|---|
| A. Формулы и короткий код | 20 | 20 мин | базовые шаблоны и Spring-01 |
| B. Чтение листинга | 20 | 20 мин | флаги, типы, адресация и массивы |
| C. Стек и функции | 25 | 25 мин | frame layout, вызовы и баланс `esp` |
| D. Reverse engineering | 20 | 15 мин | восстановление смысла без угадывания исходника |
| E. Объяснение моделей | 15 | 10 мин | security, x87 и C++ object model |
...
- **0–59** — не пытайся заучить ответы: вернись к диагностическим checkpoints по порядку.

Ошибка в обязательном инварианте — перепутанные `x/[x]`, signed/unsigned jump, несбалансированный стек или неверный размер `%f` — требует повторения темы даже при высоком общем балле.

...
- `|coordinate| <= 10000`;
- определитель помещается в 32-битные знаковые.
...
Значит числа с плавающей точкой не нужен.
...
| печатать через `%f` | числа с плавающей точкой не нужен |

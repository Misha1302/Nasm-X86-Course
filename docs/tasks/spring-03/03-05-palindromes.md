...
Даны 32-битное беззнаковое число `M` и неотрицательное число `N`.
...
| потерять `M` после вызова функции разворота | `eax/ecx/edx` изменяемые вызываемой функцией |

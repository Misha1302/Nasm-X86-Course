...
parts = [
    ...
    "",
    "Он включает самостоятельный маршрут, все 25 глав, отдельный маршрут Дня 10, задачи на перенос знаний, диагностические ключи, контрольные точки, критерии оценивания и ИИ-наставника.",
    "",
]
...
closed = [
    ...
    "",
    "> Эта страница сгенерирована автоматически и не содержит встроенных ответов. Полные объяснения остаются в `day_XX.md`.",
    "",
]
for path in DAYS:
...
migration = [
    ...
    "",
    "> Страница сгенерирована автоматически. `6/6` доказывает только наличие обязательных блоков, а не правильность содержания или сохранение знаний.",
    "",
    "| Глава | Статус | Обязательные блоки |", "|---|---|---:|", *rows, "",
    "`структура-6/6` означает только наличие шести обязательных разделов. Правильность содержания проверяют тестовые данные, исполняемые примеры и отдельное ревью.",
]
